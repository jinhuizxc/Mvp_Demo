package com.example.jh.baselibrary.widget.weekview;
...
import com.example.jh.baselibrary.R;
...
import static com.example.jh.baselibrary.widget.weekview.WeekViewUtil.isSameDay;
import static com.example.jh.baselibrary.widget.weekview.WeekViewUtil.today;
...
public class WeekView extends View {
    ...
    private int mTodayHeaderTextColor = Color.parseColor("#ffffff");
    ...
    //new attrs
    private Paint mTodayWeekLableTextPaint;
    ...
    private float mWeekLableTextHeight;
    private float mMonthTextHeight;
    private int mHeaderCowGap = 10;
    private int mTodayCircleColor = Color.rgb(34, 171, 234);//blue
    private int mTodayWeekLableTextColor = Color.rgb(34, 171, 234);//blue
    private int mMonthTextColor = Color.BLACK;
    private int mMonthTextSize = 16;
    ...
    public WeekView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            mHeaderCowGap = a.getDimensionPixelSize(R.styleable.WeekView_headerCowGap, mHeaderCowGap);
            ...
            mMonthTextSize = a.getDimensionPixelSize(R.styleable.WeekView_monthTextSize, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, mMonthTextSize, context.getResources().getDisplayMetrics()));
            ...
            mHeaderWeekLabelTextSize = a.getDimensionPixelSize(R.styleable.WeekView_weekLableTextSize, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, mHeaderWeekLabelTextSize, context.getResources().getDisplayMetrics()));
            ...
            mTodayWeekLableTextColor = a.getColor(R.styleable.WeekView_todayWeekLableTextColor, mTodayWeekLableTextColor);
            mDayBackgroundColor = a.getColor(R.styleable.WeekView_dayBackgroundColor, mDayBackgroundColor);
            mTodayCircleColor = a.getColor(R.styleable.WeekView_toadyCircleColor, mTodayCircleColor);
            ...
            mMonthTextColor = a.getColor(R.styleable.WeekView_monthTextColor, mMonthTextColor);
    ...
    private void init() {
        ...
        //Week lable paint
        mHeaderWeekLabelPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        ...
        mWeekLableTextHeight = rect.height();


        //Today circle backgroud paint
        mTodayCirclePaint = new Paint();
        mTodayCirclePaint.setColor(mTodayCircleColor);
        ...
        mHeaderWeekLabelPaint.getTextBounds("12月", 0, "12月".length(), monthRect);
        mHeaderWeekLabelPaint.setTypeface(Typeface.DEFAULT_BOLD);
        mHeaderWeekLabelPaint.setTextAlign(Paint.Align.CENTER);
        mMonthTextHeight = monthRect.height();
        ...
        mTimeTextPaint.getTextBounds("00 PM", 0, "00 PM".length(), rectTime);
        mTimeTextHeight = rectTime.height();
        ...
        mHeaderTextPaint.setTextSize(mHeaderWeekLabelTextSize);
        mHeaderTextPaint.getTextBounds("31", 0, "31".length(), rect);
        ...
        mTodayHeaderTextPaint.setTextSize(mHeaderWeekLabelTextSize);
        mTodayHeaderTextPaint.setColor(mTodayHeaderTextColor);
        mTodayHeaderTextPaint.setTextAlign(Paint.Align.CENTER);
        mTodayHeaderTextPaint.setTypeface(Typeface.DEFAULT_BOLD);


        // Prepare today week lable text color paint.
        mTodayWeekLableTextPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mTodayWeekLableTextPaint.setTextSize(mHeaderWeekLabelTextSize);
        mTodayWeekLableTextPaint.setColor(mTodayWeekLableTextColor);
        mTodayWeekLableTextPaint.setTextAlign(Paint.Align.CENTER);
        mTodayWeekLableTextPaint.setTypeface(Typeface.DEFAULT_BOLD);

        // Prepare hour separator color paint.  that's cow gap lines
    ...
    private void calculateHeaderHeight() {
        ...
        if (containsAllDayEvent) {
            ...
            mHeaderHeight = mHeaderTextHeight + mHeaderCowGap + mWeekLableTextHeight;
        }
    }
    ...
    private void drawTimeColumnAndAxes(Canvas canvas) {
        ...
        canvas.drawRect(0, mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 3, mHeaderColumnWidth, getHeight(), mHeaderColumnBackgroundPaint);

        // Clip to paint in left column only.
        canvas.clipRect(0, mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 3, mHeaderColumnWidth, getHeight(), Region.Op.REPLACE);
        ...
        for (int i = 0; i < 24; i++) {
            ...
            if (time == null)
                throw new IllegalStateException("A DateTimeInterpreter must not return null time");
            if (top < getHeight())
                canvas.drawText(time, mTimeTextWidth + mHeaderColumnPadding, top + mTimeTextHeight + mTimeTextHeight / 3, mTimeTextPaint);
        }
    }
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        int lineCount = (int) ((getHeight() - mHeaderHeight - mHeaderRowPadding * 2 -
                mHeaderMarginBottom) / mHourHeight);
        ...
        canvas.clipRect(mHeaderColumnWidth, mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 3, getWidth(), getHeight(), Region.Op.REPLACE);
        ...
        if (!mFirstVisibleDay.equals(oldFirstVisibleDay)) {
            month = mFirstVisibleDay.get(Calendar.MONTH) + 1;
        }
        for (int dayNumber = leftDaysWithGaps + 1;
            ...
            if (mWidthPerDay + startPixel - start > 0) {
                if (mShowDistinctPastFutureColor) {
                    ...
                    float startY = mHeaderHeight + mHeaderRowPadding * 2 + mTimeTextHeight / 3 + mHeaderMarginBottom + mCurrentOrigin.y;
                    ...
                    canvas.drawRect(start, mHeaderHeight + mHeaderRowPadding * 2 + mTimeTextHeight / 3 + mHeaderMarginBottom, startPixel + mWidthPerDay, getHeight(), sameDay ? mTodayBackgroundPaint : mDayBackgroundPaint);
                }
            }
            ...
            for (int hourNumber = 0; hourNumber < 24; hourNumber++) {
                float top = mHeaderHeight + mHeaderRowPadding * 2 + mCurrentOrigin.y + mHourHeight * hourNumber + mTimeTextHeight / 3 + mHeaderMarginBottom;
                if (top > mHeaderHeight + mHeaderRowPadding * 2 + mTimeTextHeight / 3 + mHeaderMarginBottom - mHourSeparatorHeight && top < getHeight() && startPixel + mWidthPerDay - start > 0) {
                    hourLines[i * 4] = start;
                    hourLines[i * 4 + 1] = top;
        ...
        //Draw the month in the top left corner
        canvas.drawText(month + "月", mHeaderColumnPadding, mHeaderHeight / 2 + mHeaderRowPadding + mMonthTextHeight / 2, mMonthTextPaint);
        ...
        for (int dayNumber = leftDaysWithGaps + 1; dayNumber <= leftDaysWithGaps + mNumberOfVisibleDays + 1; dayNumber++) {
            ...
            String weekLabel = getDateTimeInterpreter().interpretWeek(day);

            if (weekLabel == null)
                throw new IllegalStateException("A DateTimeInterpreter must not return null date");

            canvas.drawText(weekLabel, startPixel + mWidthPerDay / 2, mWeekLableTextHeight + mHeaderRowPadding, sameDay ? mTodayWeekLableTextPaint : mHeaderWeekLabelPaint);


            // Draw the day labels.
            String dayLabel = getDateTimeInterpreter().interpretDate(day);
            if (dayLabel == null)
                throw new IllegalStateException("A DateTimeInterpreter must not return null Week");

            if (sameDay)
                canvas.drawCircle(startPixel + mWidthPerDay / 2,
                        mHeaderRowPadding + mWeekLableTextHeight + mHeaderCowGap + mHeaderTextHeight / 2,
                        mHeaderTextHeight / 2 + mHeaderCowGap / 2,
                        mTodayCirclePaint
                );
            canvas.drawText(dayLabel, startPixel + mWidthPerDay / 2, mWeekLableTextHeight + mHeaderCowGap + mHeaderTextHeight + mHeaderRowPadding, sameDay ? mTodayHeaderTextPaint : mHeaderTextPaint);
        ...
        if (mEventRects != null && mEventRects.size() > 0) {
            for (int i = 0; i < mEventRects.size(); i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date) && !mEventRects.get(i).event.isAllDay()) {
                    ...
                    // Calculate top.
                    float top = mHourHeight * 24 * mEventRects.get(i).top / 1440 + mCurrentOrigin.y + mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 3 + mEventMarginVertical + mHourSeparatorHeight / 2;

                    // Calculate bottom.
                    float bottom = mEventRects.get(i).bottom;
                    bottom = mHourHeight * 24 * bottom / 1440 + mCurrentOrigin.y + mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 3 - mEventMarginVertical - mHourSeparatorHeight / 2;
        ...
        if (mEventRects != null && mEventRects.size() > 0) {
            for (int i = 0; i < mEventRects.size(); i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date) && mEventRects.get(i).event.isAllDay()) {
                    ...
                    // Calculate top.
                    float top = mHeaderRowPadding * 2 + mHeaderMarginBottom + +mTimeTextHeight / 3 + mEventMarginVertical;
    ...
    private boolean isEventsCollide(WeekViewEvent event1, WeekViewEvent event2) {
        long start1 = event1.getStartTime().getTimeInMillis();
        long end1 = event1.getEndTime().getTimeInMillis();
        long start2 = event2.getStartTime().getTimeInMillis() + 1;
        long end2 = event2.getEndTime().getTimeInMillis();
        return false;
        ...
        if (mDateTimeInterpreter == null) {
            mDateTimeInterpreter = new DateTimeInterpreter() {
                ...
                public String interpretDate(Calendar date) {
                    try {
                        SimpleDateFormat sdf = mDayNameLength == LENGTH_SHORT ? new SimpleDateFormat("dd", Locale.getDefault()) : new SimpleDateFormat("EEE M/dd", Locale.getDefault());
                ...
                public String interpretWeek(Calendar date) {
                    try {
                        SimpleDateFormat sdf = mDayNameLength == LENGTH_SHORT ? new SimpleDateFormat("EEEEE", Locale.getDefault()) : new SimpleDateFormat("EEE M/dd", Locale.getDefault());
                        return sdf.format(date.getTime()).toUpperCase();
                    } catch (Exception e) {
                        e.printStackTrace();
                        return "";
                ...
                @Override
                public String interpretMonth(Calendar date) {
                    try {
